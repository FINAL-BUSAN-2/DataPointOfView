...
def getRecommend(userEmail:str, db: Session = Depends(get_db)):
    ...
    if mem_info.mem_age == None or mem_info.mem_gen == None :
        ...
        recommendFinal = db.query(PILL_PROD.pill_nm).filter(PILL_PROD.pill_cd.in_(subquery),~PILL_PROD.pill_nm.like('%(단종)%')).order_by(PILL_PROD.pill_nm.desc()).limit(3).all()
        return({'recommend1' : recommendFinal[0].pill_nm,'recommend2' : recommendFinal[1].pill_nm,'recommend3' : recommendFinal[2].pill_nm})