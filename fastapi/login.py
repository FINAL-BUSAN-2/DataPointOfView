...
def create_routine(routine: ERoutineCreate, db: Session = Depends(get_db)):
    ...
    logger.error(f"2222222222222222222222222222222222")

    db_routine = ERTN_SETTING(
        ertn_mem="qwert0175@naver.com",  # 로그인 아이디 필요
        ertn_id=ertn_id,
    ...
    db.add(db_routine)
    logger.error(f"4444444444444444444444444444444")
    # Commit the changes to the database
    # db.commit()

    # Refresh the object to ensure it reflects the latest state in the database
    # db.refresh(db_routine)
    logger.error(f"555555555555555555555555555")
    logger.error("Routine added successfully")
