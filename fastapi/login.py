...
def getRecommend(userEmail:str, db: Session = Depends(get_db)):
    ...
    if mem_info.mem_age == None or mem_info.mem_gen == None :
        ...
        recommendFinal = db.query(PILL_PROD.PILL_PROD).filter(PILL_PROD.pill_cd.in_(subquery),~PILL_PROD.pill_nm.like('%(단종)%')).order_by(PILL_PROD.pill_nm.asc()).limit(3).all()
...
def getProdInfo(pill_nm:str, db: Session = Depends(get_db)):
    pill_info = db.query(PILL_PROD).filter(PILL_PROD.PILL_PROD == pill_nm).first()
    return pill_info